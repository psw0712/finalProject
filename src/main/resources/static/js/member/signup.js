...
const memberEamil = document.querySelector("#memberEmail");
const emailMessage = document.querySelector("#emailMessage");

// 2) 이메일이 입력 될 때 마다 유효성 검사 수행
memberEamil.addEventListener("input", e => {
...
// 인증번호 받기 버튼
const setAuthKeyBtn = document.querySelector("#authKey");

// 인증번호 입력 후 확인
const checkAuthKeytBtn = document.querySelector("#checkAuthKeyBtn");
...
// 전달받은 숫자가 10 미만인 경우(한자리) 앞에 0붙여서 반환
...
checkAuthKeytBtn.addEventListener("click", () => {
    ...
    if(authKey.value.length < 6) { // 인증번호가 제대로 입력 안된 경우
        alert("인증번호를 정확히 입력 해주세요.");
        return;
    }

    const obj = {
        "email" : memberEamil.value,
...
memberPw.addEventListener("input", e => {
    ...
    if(inputPw.trim().length === 0) {
        ...
        checkObj.memberPw = false; // 비밀번호 유효하지 않다고 표시
        memberPw.value = ""; // 처음에 띄어쓰기 입력 방지
        return;
    }
...
memberNickname.addEventListener("input", (e) => {
    ...
    if( !regExp.test(inputNickname) ) { // 유효X
...
signUpForm.addEventListener("submit", e => {
    ...
    for(let key in checkObj) { // checkObj 요소의 key 값을 순서대로 꺼내옴
        ...
        if( !checkObj[key] ) { // false인 경우 (유효하지 않음)
            ...
            document.getElementById(key).focus(); // 초전 이동
