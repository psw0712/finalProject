...
public class EmailServiceImpl implements EmailService{
	...
	// EmailConfig 설정이 적용된 객체(메일보내기 기능)
	private final JavaMailSender mailSender;
	
	// 타임리프(템플릿 엔진)을 이용해서 html 코드 -> java 변환
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			switch(htmlName) {
			case "signup" :
				subject = "[GARBAGE TIME] 회원 가입 인증번호 입니다."; break;
			}
			...
			helper.setText( loadHtml(authKey, htmlName), true ); // html 보내겠음(변경예정)
			
			// CID(Content-ID)를 이용해 메일에 이미지 첨부
			// logo 추가예정
			helper.addInline("logo", new ClassPathResource("static/images/79c0f68af57a696697a80841e19b898f.jpg"));
	...
	// HTML 파일을 읽어와서 String으로 변환 (타임리프 적용)
